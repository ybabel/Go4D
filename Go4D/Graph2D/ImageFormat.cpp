...
#define ERRORMSG(PARAM) { error(PARAM); }
  ...
  void C_Image::Init(int iWidth, int iHeight, int iBPP) {
    ...
    {
      Palette=(COLOR*)(new char[(1<<BPP)*sizeof(COLOR)]);
    }
  ...
  int C_Image::LoadBMP (char* szFileName)
  {
    ...
    // Open file.
    ifstream bmpfile (szFileName, ios::binary);
    ...
    if (bmfh.bfType!=(unsigned short)'MB') { ERRORMSG("Not a valid BMP File"); return 0; }
  ...
  int C_Image::SaveBMP (char * szFileName)
  {
    ...
    // Create file.
    ofstream bmpfile (szFileName, ios::binary);
  ...
  int C_ImageSet::LoadGIF (char * szFileName)
  {
    ...
    do
    {
      ...
      {
        switch ((unsigned int)giffile.get())
        {
